...
public class DAO {
    ...
    // ===============================================================
    // === SỬA LOGIC CHO HOME PAGE (NEW & BEST SELLER) ===
    // ===============================================================

    // 1. Lấy 4 sản phẩm mới nhất
    public List<Product> getNewProducts() {
        List<Product> list = new ArrayList<>();
        // Sắp xếp theo ID giảm dần (ngầm hiểu ID lớn là mới hơn) hoặc ngày tạo
    ...
    public List<Product> getBestSellerProducts() {
        List<Product> list = new ArrayList<>();
        // Sắp xếp theo số lượng đã bán (daBan) giảm dần
...
/** 2. Lấy danh sách đánh giá theo sản phẩm */
...
// ===================================================
    ...
    // 2. THÊM MỚI sản phẩm
    // 2. THÊM MỚI sản phẩm (Đã cập nhật đủ trường)
    public void insertProduct(String name, String image, String price, String description, String category, String stock, String sizes, String colors, String material) {
        ...
        try {
            ...
            // Các trường mới (nếu null thì lưu chuỗi rỗng)
    ...
    // 4. CẬP NHẬT (SỬA) sản phẩm (Đầy đủ 11 tham số)
    public void updateProduct(String id, String name, String image, String price, String description, String category, String stock, String sold, String sizes, String colors, String material) {
        ...
        try {
            ...
            // Các trường mới (Xử lý null để tránh lỗi)
    ...
    public boolean createOrder(Account acc, List<CartItem> items, long totalAmount, long shippingFee, String paymentMethod) {
        ...
        try {
            ...
            ps.setLong(1, acc.getCustomerId()); // maKH
            ps.setString(2, acc.getFullName()); // hoTen
            ps.setString(3, acc.getEmail());    // email
            ps.setString(4, acc.getPhone());    // soDienThoai
            ps.setString(5, acc.getAddress());  // diaChi
            ps.setDouble(6, (double) totalAmount); // tongTien
            ps.setDouble(7, (double) shippingFee); // phiShip
            ps.setString(8, paymentMethod);     // phuongThucThanhToan
            ps.setString(9, "CHO_XAC_NHAN");    // trangThai
    ...
    // 2. HÀM LẤY ĐƠN HÀNG CỦA USER
    public List<Order> getOrdersByUsername(String username) {
        List<Order> list = new ArrayList<>();
        // Join bảng để lấy đúng đơn của user đó
    ...
    // 3. HÀM MAP DỮ LIỆU (QUAN TRỌNG NHẤT: Map đúng tên cột DB sang Entity)
    private Order mapResultSetToOrder(ResultSet rs) throws SQLException {
        Order o = new Order();
        o.setOrderId(rs.getLong("maDH"));
        o.setCustomerId(rs.getLong("maKH"));
        o.setTotalAmount(rs.getLong("tongTien"));
        o.setShippingFee(rs.getLong("phiShip")); // Lấy phí ship
        o.setStatus(rs.getString("trangThai"));
        o.setPaymentMethod(rs.getString("phuongThucThanhToan"));
        o.setOrderDate(rs.getTimestamp("ngayTao")); // Cột trong DB là ngayTao
        
        // MAP CÁC CỘT THÔNG TIN GIAO HÀNG
        o.setShipName(rs.getString("hoTen"));       // DB: hoTen -> Entity: shipName
        o.setShipPhone(rs.getString("soDienThoai")); // DB: soDienThoai -> Entity: shipPhone
        o.setShipAddress(rs.getString("diaChi"));    // DB: diaChi -> Entity: shipAddress
        
        return o;
    }
    // Thêm vào DAO.java
    
    // Hàm xóa đơn hàng (cho AccountControl)
    public boolean deleteOrder(String orderId) {
        // Chỉ cho phép xóa khi trạng thái là 'Chờ xác nhận'
    ...
    public Order getOrderById(long orderId) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                return mapResultSetToOrder(rs); // (Hàm map này bạn đã có ở các bước trước)
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public List<CartItem> getOrderDetails(long orderId) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Tạo Product giả để chứa tên, ảnh, giá lúc mua
                Product p = new Product();
                p.setId(rs.getLong("maSP"));
                p.setName(rs.getString("ten"));
                p.setImage(rs.getString("hinhAnh"));
    ...
    // ==========================================================
    // ==========================================================
    // === CÁCH CUỐI: KIỂM TRA BẰNG USERNAME (CHẮC CHẮN NHẤT) ===
    // ==========================================================
    public boolean checkUserBoughtProduct(String username, long productId) {
        ...
        // JOIN 4 bảng để đi từ DonHang -> ChiTiet -> KhachHang -> NguoiDung
        // Để đảm bảo chính xác 100% người dùng đó đã mua
        String query = """
                       SELECT dh.trangThai 
                       FROM DonHang dh
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Kiểm tra trạng thái (Chấp nhận mọi biến thể của từ "Giao")
                if (status != null && (
